...
b1=xlwt.Workbook()
sheetfirst=b1.add_sheet("p10sec")

''''
splitting the data into time intervals and for every 10sec the loop is repeating and taking the values in the form of 10sec
'''
ti = []
...
for a in range(0,54):
    for b in range(0,54):
        ...
        '''
         finding the length of rows and creating the lists for doctets, realfirstpacket and duration values 
         '''
        leng = st.nrows
        docs = []
        rf1 = []
        ...
        avg = []
        '''every value is being checked in the 10sec interval range and that values are being doctects / dur val calculated'''
        ...
        for k in range(4, 16):
            ...
            for i in range(0, 3240):
                ...
                for x in range(0, len(doc)):
                    ...
                    given = dt.timedelta(0, s, ms, 0, m, h)
                    '''checking the value in 10sec range  '''
                    if given >= start and given < end:
        ...
        def calcZ(r1a2a, r1a2b, r2a2b, N):
            ...
            z = (z1a2a - z1a2b) * ((N - 3) ** 0.5) / (2 * (1 - r2a2b) * op)
